...
static Ndarray* pow_forward(Function* const p_self, const Ndarray* xs) {
  ...
  Ndarray_copy(&ys[0], Ndarray_pow(xs[0], xs[1]));
  return ys;
}
...
static Ndarray* pow_backward(Function* const p_self, const Ndarray* gys) {
  ...
  Ndarray_copy(&gxs[0], Ndarray_mul(gys[0], Ndarray_mul(*(p_self->p_io[0][1]->p_data), Ndarray_pow(*(p_self->p_io[0][0]->p_data), Ndarray_constant_add(*(p_self->p_io[0][1]->p_data), -1)))));
  Ndarray_copy(&gxs[1], Ndarray_mul(gys[0], Ndarray_mul(*(p_self->p_io[1][0]->p_data), Ndarray_map(*(p_self->p_io[0][0]->p_data), log))));
  return gxs;
}
