...
static Ndarray* div_forward(Function* const p_self, const Ndarray* xs) {
  ...
  Ndarray_copy(&ys[0], Ndarray_div(xs[0], xs[1]));
  return ys;
}
...
static Ndarray* div_backward(Function* const p_self, const Ndarray* gys) {
  ...
  Ndarray_copy(&gxs[0], Ndarray_div(gys[0], *(p_self->p_io[0][1]->p_data)));
  Ndarray_copy(&gxs[1], Ndarray_mul(gys[0], Ndarray_neg(Ndarray_div(*(p_self->p_io[0][0]->p_data), Ndarray_mul(*(p_self->p_io[0][1]->p_data), *(p_self->p_io[0][1]->p_data))))));
  return gxs;
}
