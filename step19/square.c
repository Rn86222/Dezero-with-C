...
static Ndarray* square_forward(Function* const p_self, const Ndarray* xs) {
  Ndarray* ys;
  ys = (Ndarray*)malloc(p_self->input_num * sizeof(Ndarray));
  Ndarray_copy(&ys[0], Ndarray_mul(xs[0], xs[0]));
  return ys;
}
...
static Ndarray* square_backward(Function* const p_self, const Ndarray* gys) {
  ...
  Ndarray_copy(&gxs[0], Ndarray_constant_mul(Ndarray_mul(*(p_self->p_io[0][0]->p_data), gys[0]), 2));
  return gxs;
}
